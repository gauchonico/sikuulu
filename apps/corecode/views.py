...
class CurrentSessionAndTermView(LoginRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            # Update current session: set the selected session to current=True
            AcademicSession.objects.filter(name=session).update(current=True)
            
            # Set all other sessions to current=False
            AcademicSession.objects.exclude(name=session).update(current=False)
            
            # Update current term: set the selected term to current=True
            AcademicTerm.objects.filter(name=term).update(current=True)
            
            # Set all other terms to current=False
